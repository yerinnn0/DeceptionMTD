...
class PolicyOptimizationGurobi(PolicyOptimization):
    ...
    def solve_MDP(self):
        ...
        print("Solving MDP with Gurobi...")
        
        # Example problem dimensions
        n = self.n_states * self.n_actions

        # Concave quadratic (Q is negative definite)
        q = - self.r.flatten()
        ...
        # Set objective: 0.5 * x^T P x + q^T x
        quad_obj = q @ x
        model.setObjective(quad_obj, GRB.MINIMIZE)

        # Add constraints
        model.addConstr(self.G @ x <= self.h, name="ineq")
        model.addConstr(self.A @ x == self.b, name="eq")
        ...
        if model.status == GRB.OPTIMAL:
            ...
            print("Problem status:", model.status)
    ...
    def diversionary_deception(self, original_occupancy_measures, init= None, beta=1):
        
        print("Solving Diversionary Deception with Gurobi...")
        
        if isinstance(original_occupancy_measures, torch.Tensor):
            original_occupancy_measures = original_occupancy_measures.cpu().numpy()
        
        if beta == 0:
            return self.evaluation(original_occupancy_measures.flatten())
        ...
        # Concave quadratic (Q is negative definite)
        P  = -2*beta*sp.identity(n, format='coo')
        q = (2 * beta) * original_occupancy_measures.flatten() - self.r.flatten()
    ...
    def targeted_deception(self, target_occupancy_measure, initvals= None, beta=1):
        
        print("Solving Targeted Deception with Gurobi...")
        ...
        # Concave quadratic (Q is negative definite)
        P  = -2*beta*sp.identity(n, format='coo')
        q = (2 * beta) * target_occupancy_measure.flatten() - self.r.flatten()
    ...
    def equivocal_deception(self, original_occupancy_measures = None, initvals= None, beta=1):
        
        print("Solving Equivocal Deception with Gurobi...")
